...
このようにダンボールなどの板に貼り付け、きれいな平面の状態を保てるようにしておくとカメラ校正を行いやすいですし、後々まで使い続けることができます。  
...
3. Diamondクラスターを使うやり方  
    [こちらのzip](https://github.com/Akiya-Research-Institute/MocapForAll-Wiki/raw/main/resources/calibration/DiamondMarkers.zip)にある"diamondMarker0.png"と"同1"を使います。「1.ChArUcoボードを使うやり方」と同様に、**可能であればA2以上**のサイズで印刷してください。  
...
2. ArUcoクラスターを使う方法  
    [外部パラメータの取得で使う画像の印刷](#外部パラメータの取得で使う画像の印刷)で印刷した3種類の画像を床に置きます。「arucoMarker0.png」がモーションキャプチャの原点になります。  
    床に置いた画像が見える場所にカメラを設置します。  
...
3. Diamondクラスターを使う方法  
    ...
    MocapForAllで「設定 > カメラ校正 > カメラ外部パラメータの求め方」で「**Diamond cluster**」を選択します。
