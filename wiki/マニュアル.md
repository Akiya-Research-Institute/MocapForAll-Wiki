...
## カメラ校正
### カメラ校正とは？
カメラ校正は、カメラ画像中での位置と現実世界での位置を対応付けるための情報を取得する作業です。この節では、カメラ校正の概要といくつかの注意点を説明します。  
カメラ構成で何をやっているのかの概念など、技術的に詳しく知り理解を深めたい方は詳細をご覧ください。
...
カメラ（とレンズ)に固有のもので、基本的に変化しません。そのため、**同じカメラを使う限り一度正しく取得すればそれ以降は再取得は不要**になります。
